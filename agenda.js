...
bot.start(async ctx => {
    ...
    await ctx.replyWithMarkdown(`Seja bem vindo, ${nome}!\n👋`)
    await ctx.reply(`Eu sou o bot Julho. Um bot capaz de guardar as suas tarefas, como uma agenda.`)
    await ctx.reply(`Se precisar de algo, digite /ajuda`)
    await ctx.replyWithMarkdown(`Me envie suas tarefas que irei armazená-las.\n\nComeçe com o nome dela.`)
})
...
bot.command('ajuda', async ctx => {
    ctx.replyWithMarkdown(`/dia  ->   ver a sua agenda do dia.\n/amanha  ->  mostrar todas tarefas pendentes até amanhã.\n/semana  ->  mostrar todas tarefas da semana.\n/concluidas -> mostrar todas tarefas já concluídas.\n/tarefas  ->  mostrar tarefas sem data de previsão para término definida.\n\nObs: Digite o nome da tarefa para começar a adicioná-la.`)
})
